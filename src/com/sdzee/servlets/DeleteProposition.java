 ...
 * 	<li>Servlet qui supprime une proposition d'une question</li>
 * </ul>
 * 
 ...
 * @author Huihui Huang
 */
public class DeleteProposition extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 1. Obtenir l'ID de la catégorie et proposition
		Integer id= Integer.parseInt(request.getParameter("id"));
		Integer idCategorie= Integer.parseInt(request.getParameter("idCategorie"));
		
		// 2. Trouver la proposition via son ID
		Proposition proposition= propositionDao.trouver(id);
		...
		try {	
			if (proposition !=null) {
				...
				// 4. Trouver la liste de réponses via la proposition
				List<Reponse> reponses= reponseDao.trouver(proposition);
							
				// Parcourir la liste de réponses
				for (Reponse reponse: reponses) {
					// 5. Supprimer la réponse
					reponseDao.supprimerReponse(reponse);
				}
							
				// 6. Supprimer la proposition
				propositionDao.supprimer(proposition);
		
				resultat="<article class=\"succes\">Succès de la suppression de la proposition.</article>";
			} else {
				resultat="<article class=\"erreur\">Échec de la suppression de la proposition.</article>";
			}

		}  catch ( DAOException e ) {
			resultat = "<article class=\"erreur\">Échec de la suppresion de la proposition : une erreur imprévue est survenue, cela semble être un problème de communication avec la base de données du site. Merci de réessayer dans quelques instants.</article>";
			e.printStackTrace();
		}
