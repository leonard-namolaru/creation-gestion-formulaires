...
public class DeleteGroupe extends HttpServlet {
	 ...
	 * @see CategorieDao#supprimerAll(Categorie)
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Integer numeroGroupe = Integer.parseInt(request.getParameter("groupe"));
		...
		List<Question> questions = questionDao.lister(categorie, numeroGroupe);
		...
		try {	
			if (categorie != null) {
				...
				resultat="<article class=\"succes\">Succès de la suppression du groupe.</article>";
			} else {
				resultat="<article class=\"erreur\">Échec de la suppression du groupe.</article>";
			}

		}  catch ( DAOException e ) {
			resultat = "<article class=\"erreur\">Échec de la suppresion du groupe : une erreur imprévue est survenue, cela semble être un problème de communication avec la base de données du site. Merci de réessayer dans quelques instants.</article>";
			e.printStackTrace();
		}
